...
def config(**changes):
    return agents.AgentInput.model_validate({"name": "Reader", "role": "Research",
        "instructions": "Cite evidence.", "modelId": None, "toolIds": ["read", "search"],
        "memory": {"scope": "selected", "memoryIds": ["one", "two"]}, **changes})
...
def team(identity):
    ...
    return c.Team.model_validate({"name": "Review team", "objective": "Review evidence",
        "roles": [{"id": "reader", "name": "Reader", "agentId": identity,
        "instructions": "Report evidence.", "toolIds": ["read"],
        "memory": {"scope": "selected", "memoryIds": ["one"]},
        "context": {"mode": "task_only", "sourceIds": []}, "budget": budget}],
        "handoffs": [], "budget": {**budget, "maxHandoffs": 0}})
...
def test_durable_template_versions_and_whole_field_replacements(tmp_path):
    ...
    with closing(Store(path)) as store:
        ...
        prepared = c.prepare(store, identity, "template", c.Instantiate(expected_revision=2,
            version=1, name="New reader", overrides={"toolIds": [], "memory": {"scope": "none", "memoryIds": []}}))
        ...
        c.save(store, "template", template().model_copy(update={"description": "Changed"}), identity, 2)
        c.archive(store, identity, "template", agents.ArchiveAgent(expected_revision=3, archived=True))
        with pytest.raises(agents.AgentError, match="Restore"):
            c.prepare(store, identity, "template", c.Instantiate(expected_revision=4, version=1, name="Other"))
        c.archive(store, identity, "template", agents.ArchiveAgent(expected_revision=4, archived=False))
...
def test_publish_cas_and_duplicate_publication(store):
    identity = c.save(store, "template", template())["id"]
    def attempt(_):
        try: return c.publish(store, identity, c.Revision(expected_revision=1))
        except agents.AgentError as exc: return exc.detail
...
def test_team_snapshots_revalidate_agent_changes_without_widening(store):
    ...
    agents.update(store, agent["id"], agents.UpdateAgent(expected_revision=1, configuration=config(toolIds=[])))
...
@pytest.mark.parametrize("mutation", ["role_id", "role_name", "turns", "tokens", "cost", "edge_self", "edge_missing", "edge_pair", "transfers", "context", "strict_budget"])
def test_team_graph_and_budget_validation(mutation):
    ...
    elif mutation == "context": value["roles"][0]["context"]["sourceIds"] = ["source"]
    elif mutation == "strict_budget": value["budget"]["maxTurns"] = True
    else:
        value["roles"].append({**copy.deepcopy(value["roles"][0]), "id": "writer", "name": "Writer"})
        value["budget"].update(maxTurns=4, maxTokens=200, maxHandoffs=2)
        edge = {"id": "handoff", "fromRoleId": "reader", "toRoleId": "writer", "condition": "Ready", "payload": "result_only", "maxTransfers": 1}
        if mutation == "edge_self": edge["toRoleId"] = "reader"
        if mutation == "edge_missing": edge["toRoleId"] = "missing"
        if mutation == "transfers": edge["maxTransfers"] = 3
        value["handoffs"] = [edge]
        if mutation == "edge_pair": value["handoffs"].append({**edge, "id": "second"})
    with pytest.raises(ValidationError): c.Team.model_validate(value)
...
def test_removal_tombstone_and_name_reuse(store):
    ...
    with pytest.raises(agents.AgentError): c.get(store, item["id"], "template")
    assert c.save(store, "template", template())["id"] != item["id"]
    with store._connect() as db:
        assert db.execute("SELECT deleted_at FROM collaboration_records WHERE id=?", (item["id"],)).fetchone()[0]


def test_snapshots_sql_immutable(store):
    item = c.save(store, "template", template())
    c.publish(store, item["id"], c.Revision(expected_revision=1))
    c.prepare(store, item["id"], "template", c.Instantiate(expected_revision=2, version=1, name="Prepared"))
    with store._connect() as db:
...
def test_http_admin_and_bad_overrides(store, monkeypatch):
    ...
    monkeypatch.setattr(api.app.state, "gateway_key_hash", hashlib.sha256(b"runtime_test_key").hexdigest(), raising=False)
    client = TestClient(api.app)
    headers = {"X-Pi-Key": "owner_admin_test_key"}
    route = "/collaboration/templates"
    assert client.get("/collaboration").status_code == 401
    assert client.post(route, json=template().model_dump(), headers={"X-Pi-Gateway-Key": "runtime_test_key"}).status_code == 403
    item = client.post(route, json=template().model_dump(), headers=headers).json()
    path = route + "/" + item["id"]
    assert client.post(path + "/publish", json={"expected_revision": 1}, headers=headers).status_code == 200
    assert client.post(path + "/instantiate", json={"expected_revision": 2, "version": 1,
        "name": "Prepared", "overrides": {"memory": {"scope": "none"}}}, headers=headers).status_code == 422
...
def test_team_bounded_cycle_update_archive_and_durable_snapshot(tmp_path):
    ...
    with closing(Store(path)) as store:
        ...
        value["roles"].append({**copy.deepcopy(value["roles"][0]), "id": "writer", "name": "Writer"})
        value["budget"].update(maxTurns=4, maxTokens=200, maxHandoffs=2)
        value["handoffs"] = [{"id": "forward", "fromRoleId": "reader", "toRoleId": "writer",
            "condition": "Evidence ready", "payload": "result_and_citations", "maxTransfers": 1},
            {"id": "back", "fromRoleId": "writer", "toRoleId": "reader",
            "condition": "Needs review", "payload": "result_only", "maxTransfers": 1}]
        ...
        archived = c.archive(store, record["id"], "team", agents.ArchiveAgent(expected_revision=2, archived=True))
