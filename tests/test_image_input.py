...
from pi import attachment_turns, attachments, context_controls, image_input, session_settings, turn_context
...
@pytest.mark.parametrize("format,media", [("PNG", "image/png"), ("JPEG", "image/jpeg"), ("WEBP", "image/webp")])
...
def test_real_adapters_transmit_image_in_provider_format(monkeypatch, kind):
    ...
    def post(url, **kwargs):
        ...
        body = ({"content": [{"type": "text", "text": "red"}]} if kind == "anthropic" else
                {"message": {"content": "red"}} if kind == "ollama" else
                {"choices": [{"message": {"content": "red"}}]})
        return httpx.Response(200, json=body, request=httpx.Request("POST", url))

    monkeypatch.setattr(httpx, "post", post)
    adapter = {"openai": lambda: OpenAIProvider("synthetic", allow_paid=True),
               "anthropic": lambda: AnthropicProvider("synthetic", allow_paid=True),
               "openrouter": lambda: OpenRouterProvider("synthetic"),
               "ollama": lambda: OllamaProvider("http://synthetic")}[kind]()
    if kind == "openrouter":
        monkeypatch.setattr(adapter, "_guard_cost", lambda model: SimpleNamespace(supports_images=True))
    assert adapter.complete([Message("user", "Describe", (image,))], model="synthetic").text == "red"
    message = sent[0]["messages"][0]
    if kind == "ollama":
        assert message["images"] == [image.data]
    elif kind == "anthropic":
        assert message["content"][0]["source"] == {"type": "base64", "media_type": "image/png", "data": image.data}
    else:
...
def test_image_turn_and_original_context_replay_preserve_image(tmp_path):
    with closing(Store(tmp_path / "pi.db")) as store:
        ...
        raw = picture()
        item = attachments.upload(store, sid, attachments.Metadata(name="image.png", type="image/png"),
                                  raw, resolve=session_settings.source_privacy)
        calls = []
        ...
        result = loop.run_turn(sid, "Describe", request_id="image_request_01", attachment_ids=[item["id"]])
        ...
        context_controls.save(store, sid, context_controls.Update(expected_revision=0,
            policy=context_controls.Policy(sessionInstructions="", messagePolicies={source: "exclude"},
                budget=context_controls.Budget(contextWindowTokens=20000, outputReserveTokens=100,
                                               otherInputTokens=0))))
        loop.run_turn(sid, "New question", request_id="image_request_02")
        assert not any(message.images for message in calls[-1])
        assert [image for message in turn_context.replay(store, result["turn_id"])["messages"]
                for image in message.images] == images
...
def test_private_image_cannot_be_reused_with_weaker_privacy(tmp_path):
    with closing(Store(tmp_path / "pi.db")) as store:
        ...
        session_settings.save(store, sid, session_settings.Update(expected_revision=0,
            settings=session_settings.Settings(agentId="companion",
                privacy=session_settings.Privacy(memoryDisabled=True, harnessDisabled=False))))
        item = attachments.upload(store, sid, attachments.Metadata(name="private.png", type="image/png"),
                                  picture(), resolve=session_settings.source_privacy)
        with store._connect() as db, pytest.raises(attachments.AttachmentError) as error:
            attachment_turns._text(db, sid, item["id"], {"memoryDisabled": False, "harnessDisabled": False})
...
def test_text_only_catalogue_model_never_receives_image(monkeypatch):
    ...
    monkeypatch.setattr(adapter, "_guard_cost", lambda model: SimpleNamespace(supports_images=False))
    monkeypatch.setattr(httpx, "post", lambda *a, **kw: pytest.fail("Image sent to text-only model"))
    image = image_input.prepare(picture(), "image/png", "attachment_a")
