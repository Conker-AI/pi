...
class Adapter:
    ...
    def complete_bounded(self, messages, *, model, timeout):
        ...
        if self.fail: raise ProviderUnavailable("Unavailable")
...
def config(primary="a", mode="manual"):
    empty = dict(enabled=False, eligibleModelIds=[], modelId=None, timeoutMs=1700, failure="stop", fallbackModelId=None)
    answer = {**empty, "enabled": True, "eligibleModelIds": ["a", "b"], "modelId": primary}
    return model_roles.Configuration.model_validate({
        "providers": [{"id": "local", "name": "Local", "enabled": True}, {"id": "hosted", "name": "Hosted", "enabled": True}],
        "models": [{"id": "a", "providerId": "local", "name": "A", "route": "actual-a", "enabled": True},
                   {"id": "b", "providerId": "hosted", "name": "B", "route": "actual-b", "enabled": True}],
        "defaultModelId": "a", "roleSettings": {"answerMode": mode,
            "roles": {**{role: dict(empty) for role in model_roles.ROLES}, "answer": answer,
                      "routing": answer if mode == "router" else dict(empty)}}})


def save(store, value):
    return model_roles.save(store, model_roles.Update(expected_revision=model_roles.load(store)["revision"], configuration=value))


def private(store, sid, agent="companion"):
    session_settings.save(store, sid, session_settings.Update(expected_revision=0,
        settings=session_settings.Settings(agentId=agent, privacy=session_settings.Privacy(memoryDisabled=True, harnessDisabled=True))))


def test_frozen_role_configuration_and_actual_provider_evidence(tmp_path):
    with closing(Store(tmp_path / "test.db")) as store:
        sid = store.create_session()
        save(store, config(primary="b"))  # defaultModelId remains a; role assignment is authoritative.
        ...
        loop = Loop(store, Router(local_provider=local, hosted_provider=hosted, local_model="legacy"))
...
def test_manual_lock_failure_never_falls_back(tmp_path):
    with closing(Store(tmp_path / "test.db")) as store:
        ...
        with pytest.raises(TurnFailed, match="unapproved substitute"):
            Loop(store, Router(local_provider=local, hosted_provider=hosted, local_model="legacy")).run_turn(sid, "Question")
...
def test_agent_override_uses_catalogue_and_skips_helper(tmp_path):
    with closing(Store(tmp_path / "test.db")) as store:
        sid = store.create_session()
        agent = agents.create(store, agents.AgentInput(name="Reader", role="Research", instructions="Cite evidence.",
            modelId="b", toolIds=[], memory=agents.MemorySelection(scope="none", memoryIds=[])))
...
def test_execution_identity_cannot_cross_sessions(tmp_path):
    with closing(Store(tmp_path / "test.db")) as store:
        ...
        receipt = submissions.bind(store, request)
        for kwargs in ({"turn_id": receipt["turn_id"]}, {"request_id": request}, {"turn_id": "missing"}):
...
def test_configured_summary_role_and_privacy(tmp_path):
    with closing(Store(tmp_path / "test.db")) as store:
        sid = store.create_session()
        value = config()
        value.roleSettings.roles["summarization"] = value.roleSettings.roles["answer"].model_copy(update={"modelId": "b"})
        save(store, value)
