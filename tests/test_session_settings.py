...
from pi import agents, api, context_controls, memory_store, session_settings as settings, submissions
...
def change(store, sid, memory=False, harness=False, agent="companion"):
    return settings.save(store, sid, settings.Update(expected_revision=settings.load(store, sid)["revision"],
        settings=settings.Settings(agentId=agent, privacy=settings.Privacy(memoryDisabled=memory, harnessDisabled=harness))))


def config(**overrides):
    return agents.AgentInput.model_validate({"name": "Reader", "role": "Research", "instructions": "Use only cited evidence.",
        "modelId": None, "toolIds": ["read"], "memory": {"scope": "none", "memoryIds": []}, **overrides})


def client(handler):
    return MemoryClient("http://memory.test", "ingest_test", "read_test", transport=httpx.MockTransport(handler))
...
def test_future_privacy_retains_prior_queue_and_never_backfills_private_text(tmp_path):
    ...
    with closing(Store(path)) as store:
        ...
        with store._connect() as db:
            assert [row[0] for row in db.execute("SELECT message_id FROM memory_outbox")] == [prior["id"]]
        change(store, sid)
        with store._connect() as db:
            assert settings.source_privacy(db, sid)["memoryDisabled"] is True
    with closing(Store(path)) as store:
        with store._connect() as db:
            assert not db.execute("SELECT 1 FROM memory_outbox WHERE message_id=?", (private["id"],)).fetchone()
            # Even an erroneous queue insertion cannot bypass the delivery gate.
            db.execute("INSERT INTO memory_outbox(message_id,operation) VALUES (?,'ingest')", (private["id"],))
...
def test_no_memory_turn_skips_remote_retrieval_and_delivery(tmp_path):
    with closing(Store(tmp_path / "test.db")) as store:
        ...
        provider = Provider()
        result = Loop(store, Router(local_provider=provider, local_model="fixed"), memory=memory).run_turn(sid, "Private")
        ...
        with store._connect() as db:
            ...
            db.execute("INSERT INTO memory_outbox(message_id,operation) VALUES (?,'ingest')", (message_id,))
        assert memory.drain_once() == 0 and calls == []
...
def test_settings_freeze_at_reservation_and_block_changes(tmp_path):
    with closing(Store(tmp_path / "test.db")) as store:
        ...
        with pytest.raises(agents.AgentError, match="Resolve"):
            change(store, sid)
        agents.update(store, agent["id"], agents.UpdateAgent(expected_revision=1, configuration=config(instructions="Later edit")))
        receipt = submissions.bind(store, request)
        frozen = settings.execution(store, sid, receipt["turn_id"])
        assert frozen["agentVersion"] == 1 and frozen["configuration"]["instructions"] == "Use only cited evidence."
        with pytest.raises(agents.AgentError, match="Resolve"):
            change(store, sid)
...
def test_fork_inherits_settings_and_harness_preserves_tools(tmp_path):
    with closing(Store(tmp_path / "test.db")) as store:
        ...
        class Tools:
            def tools(self): return [Tool("read", "Read", "Read a selected source", [])]
...
def test_agent_instructions_tool_intersection_and_unmapped_model_fail_closed(tmp_path):
    with closing(Store(tmp_path / "test.db")) as store:
        ...
        provider = Provider()
        class Tools:
            def tools(self): return [Tool("read", "Read", "Allowed", []), Tool("write", "Write", "Unselected", [])]
...
def test_reviewed_fork_inherits_private_settings(tmp_path):
    with closing(Store(tmp_path / "test.db")) as store:
        sid = store.create_session()
        change(store, sid, memory=True, harness=True)
        policy = context_controls.Policy(sessionInstructions="Retain this instruction", messagePolicies={},
            budget=context_controls.Budget(contextWindowTokens=1000, outputReserveTokens=100, otherInputTokens=0))
        context_controls.save(store, sid, context_controls.Update(expected_revision=0, policy=policy))
        fork = context_controls.reviewed_fork(store, sid, context_controls.ReviewedFork(expected_revision=1,
            expected_last_message_id=None, summary="Owner reviewed context", request_id="reviewed_privacy_fork_01"))
        assert settings.load(store, fork["session_id"])["settings"] == settings.load(store, sid)["settings"]
