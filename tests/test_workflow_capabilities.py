...
def test_workflow_capability_pins_dispatch_and_checks_receipt(monkeypatch):
    ...
    responses = [
        {"code": "CONFIRMATION_REQUIRED", "request_id": "request-1"},
        {
            ...
            "status": "completed",
            "action_id": "pi_exact",
            "publication": {"id": "daily.brief", "version": 3, "digest": digest},
            "result": {"answer": 7},
        },
    ]
    ...
    wrong = httpx.Response(
        200,
        json={
            "code": "OK",
            "status": "completed",
            "action_id": "pi_exact",
            "publication": {"id": "daily.brief", "version": 4, "digest": digest},
            "result": {},
        },
    )
...
def test_workflow_catalogue_joins_tools_without_leaking_execution_metadata(monkeypatch):
    ...
    def get(url, **kwargs):
        rows = (
            ...
            if url.endswith("published-workflows")
            else [{"id": "echo", "name": "Echo"}]
        )
        return httpx.Response(200, json=rows, request=httpx.Request("GET", url))

    monkeypatch.setattr(httpx, "get", get)
    assert [tool.id for tool in ToolGateClient("http://gate", "execution-only").tools()] == [
        "echo",
        alias,
    ]
