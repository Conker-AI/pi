...
@pytest.mark.parametrize("path,value", [
    (("idleTimeoutMinutes",), True), (("idleTimeoutMinutes",), 7),
    (("urgency",), "all"), (("unexpected",), True),
    (("quietHours", "enabled"), 1), (("quietHours", "start"), "24:00"),
    (("quietHours", "end"), "22:00"), (("quietHours", "timeZone"), "not/a-zone"),
    (("dailyBudget", "costCents"), -1), (("dailyBudget", "costCents"), 1.5),
    (("dailyBudget", "suggestions"), 101), (("dailyBudget", "researchMinutes"), 1441),
])
...
def test_dst_repeat_skip_and_day_boundary():
    value = settings(quietHours={"enabled": True, "start": "01:00", "end": "03:00",
                                "timeZone": "America/New_York", "urgentExceptions": False})
    ...
    assert prefs.local_day(value, datetime.fromisoformat("2026-01-02T02:00:00+00:00")) == "2026-01-01"
...
def test_urgency_budget_and_stale_usage():
    ...
    value = settings(urgency="off", quietHours={**prefs.DEFAULT["quietHours"], "urgentExceptions": True})
    kwargs = dict(urgent=True, usage_day="2026-01-02", used=zero,
                  proposed=prefs.Usage(suggestions=1, researchMinutes=1, costCents=1))
    assert prefs.admission_reasons(value, instant, **kwargs) == ["proactivity_off", "daily_budget_costCents"]
    value = settings(urgency="urgent_only", dailyBudget=dict(suggestions=0, researchMinutes=0, costCents=0))
    reasons = prefs.admission_reasons(value, instant, **{**kwargs, "urgent": False})
    assert reasons == ["urgent_only", "quiet_hours", "daily_budget_suggestions",
                       "daily_budget_researchMinutes", "daily_budget_costCents"]
    with pytest.raises(ValueError):
...
def test_admin_only_http_and_failed_save_atomic(tmp_path, monkeypatch):
    with closing(Store(tmp_path / "test.db")) as store:
        ...
        for method in ("get", "post"):
            ...
            assert getattr(client, method)(route, headers={"X-Pi-Gateway-Key": "runtime_test_key"}, **args).status_code == 403
        ...
        assert client.post(route, headers=headers, json={**body, "expected_revision": True}).status_code == 422
...
def test_daytime_window_disabled_and_accumulated_budget():
    ...
    kwargs = dict(urgent=False, usage_day="2026-01-01",
    ...
    assert prefs.admission_reasons(value, instant, **kwargs) == [
        "daily_budget_suggestions", "daily_budget_researchMinutes"]
