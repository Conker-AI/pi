...
def configuration(**changes):
    return agents.AgentInput.model_validate({"name": "Researcher", "role": "Review evidence",
        "instructions": "Cite evidence and report uncertainty.", "modelId": None,
        "toolIds": [], "memory": {"scope": "conversation", "memoryIds": []}, **changes})
...
def test_durability_history_archive_and_restore(tmp_path):
    ...
    with closing(Store(path)) as store:
        ...
        edited = agents.update(store, identity, agents.UpdateAgent(expected_revision=1,
            configuration=configuration(name="Evidence reader")))
        archived = agents.archive(store, identity, agents.ArchiveAgent(expected_revision=2, archived=True))
        ...
        with pytest.raises(agents.AgentError, match="Restore"):
            agents.update(store, identity, agents.UpdateAgent(expected_revision=3, configuration=configuration()))
        assert agents.archive(store, identity, agents.ArchiveAgent(expected_revision=3, archived=True)) == archived
        restored = agents.archive(store, identity, agents.ArchiveAgent(expected_revision=3, archived=False))
    ...
    with closing(Store(path)) as store:
        ...
        assert [item["change_kind"] for item in history["results"]] == ["created", "updated", "archived", "restored"]
        assert len([item for item in agents.list_agents(store)["results"] if item["kind"] == "companion"]) == 1
...
def test_cas_race_and_atomic_name_collision(store):
    ...
    def attempt(name):
        try:
            return agents.update(store, first["id"], agents.UpdateAgent(expected_revision=1,
    ...
    with pytest.raises(agents.AgentError, match="unique"):
        agents.update(store, first["id"], agents.UpdateAgent(expected_revision=2,
            configuration=configuration(name="OTHER")))
...
def test_singular_companion_and_archived_name_protection(store):
    for action in (lambda: agents.create(store, configuration(name="CONKER")),
                   lambda: agents.update(store, "companion", agents.UpdateAgent(expected_revision=1, configuration=configuration())),
                   lambda: agents.archive(store, "companion", agents.ArchiveAgent(expected_revision=1, archived=True))):
...
@pytest.mark.parametrize("changes", [
    {"name": " "}, {"name": "a" * 81}, {"role": "a" * 161},
    {"instructions": "a" * 8001}, {"modelId": ""}, {"modelId": 1},
    {"toolIds": ["x", "x"]}, {"toolIds": [" "]}, {"toolIds": [1]},
    {"memory": {"scope": "selected", "memoryIds": []}},
    {"memory": {"scope": "none", "memoryIds": ["x"]}},
    {"memory": {"scope": "all", "memoryIds": []}},
    {"kind": "companion"}, {"grants": ["admin"]},
])
...
def test_selections_are_not_authority_and_snapshots_cannot_be_rewritten(store):
    item = agents.create(store, configuration(modelId="requested-model", toolIds=["requested-tool"],
    ...
    statements = [
        ...
        ("INSERT OR REPLACE INTO agent_versions SELECT * FROM agent_versions WHERE agent_id=?", (item["id"],)),
    ]
...
def test_http_admin_only_and_validation(store, monkeypatch):
    ...
    identity = item["id"]
    requests = [("get", "/agents", None), ("post", "/agents", configuration().model_dump()),
        ("get", f"/agents/{identity}", None), ("get", f"/agents/{identity}/versions", None),
        ("get", f"/agents/{identity}/versions/1", None),
        ("post", f"/agents/{identity}/update", {"expected_revision": 1, "configuration": configuration().model_dump()}),
    ...
    for method, path, body in requests:
        ...
        assert getattr(client, method)(path, headers={"X-Pi-Gateway-Key": "runtime_test_key"}, **args).status_code == 403
    ...
    assert client.post(f"/agents/{identity}/archive", headers=headers,
        json={"expected_revision": True, "archived": True}).status_code == 422
