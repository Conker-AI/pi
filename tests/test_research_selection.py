...
def test_mode_frozen_in_submission_and_turn_after_reopen(tmp_path, mode):
    ...
    with closing(Store(path)) as store:
        ...
        receipt, created = submissions.reserve(store, "research_request_001", sid, "Find sources", {}, research_mode=mode)
        assert created and receipt["research_mode"] == mode
        assert not submissions.reserve(store, "research_request_001", sid, "Find sources", {}, research_mode=mode)[1]
...
def test_unavailable_runtime_retains_intent_without_dispatch_or_silent_fallback(tmp_path):
    with closing(Store(tmp_path / "pi.db")) as store:
        ...
        with pytest.raises(tasks.TaskError) as failure:
            loop.run_turn(sid, "Investigate", request_id="research_failure_001", research_mode="deep")
        ...
        replay = loop.run_turn(sid, "Investigate", request_id="research_failure_001", research_mode="deep")
...
def test_queue_preserves_mode_on_edit_review_and_requires_exact_admission(tmp_path):
    with closing(Store(tmp_path / "pi.db")) as store:
        sid = store.create_session()
        entry = q.enqueue(store, sid, q.Enqueue(request_id="research_queue_001", text="Find sources", research_mode="web"))
        entry = q.change(store, sid, entry["id"], q.Edit(expected_revision=1, text="Find more sources"), "edit")
        ...
        with pytest.raises(tasks.TaskError):
            submissions.reserve(store, rid, sid, "Find more sources", {}, queued_entry=(entry["id"], entry["revision"]))
        assert q.read(store, sid)["entries"][0]["state"] == "waiting"
        receipt, _ = submissions.reserve(store, rid, sid, "Find more sources", {}, queued_entry=(entry["id"], entry["revision"]), research_mode="web")
...
def test_off_is_legacy_compatible_and_changes_no_permissions(tmp_path):
    with closing(Store(tmp_path / "pi.db")) as store:
        ...
        replay, created = submissions.reserve(store, "ordinary_request_001", sid, "Hello", {}, research_mode="off")
...
def test_draft_mode_survives_reopen_and_newer_draft_survives_binding(tmp_path):
    ...
    with closing(Store(path)) as store:
        sid = store.create_session()
        drafts.save(store, sid, drafts.Save(expected_revision=0, text="Research this", research_mode="web"))
    with closing(Store(path)) as store:
        assert drafts.load(store, sid)["research_mode"] == "web"
        with pytest.raises(submissions.SubmissionError):
            submissions.reserve(store, "research_draft_001", sid, "Research this", {}, draft_revision=1)
        submissions.reserve(store, "research_draft_001", sid, "Research this", {}, draft_revision=1, research_mode="web")
...
def test_consumed_draft_clears_mode_with_text(tmp_path):
    with closing(Store(tmp_path / "pi.db")) as store:
        sid = store.create_session()
        drafts.save(store, sid, drafts.Save(expected_revision=0, text="Research this", research_mode="deep"))
        submissions.reserve(store, "research_draft_002", sid, "Research this", {}, draft_revision=1, research_mode="deep")
...
def test_http_research_selection_is_forwarded_and_receipt_explains_unavailability(tmp_path, monkeypatch):
    with closing(Store(tmp_path / "pi.db")) as store:
        sid = store.create_session()
        provider = Recorder()
        ...
        monkeypatch.setattr(api.app.state, "gateway_key_hash", hashlib.sha256(key.encode()).hexdigest(), raising=False)
        client = TestClient(api.app)
        try:
            payload = {"text": "Investigate", "request_id": "research_http_001", "research_mode": "web"}
