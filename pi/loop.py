...
class Loop:
    def __init__(
        ...
        *,
        ...
        toolgate: ToolGateClient | None = None,
    ...
    def _call_once(self, messages: list[Message], ctx: TurnContext, execution=None, role="answer"):
        ...
        override = (
            (
                execution.get("answerModelId")
                or (execution.get("configuration") or {}).get("modelId")
            )
            if execution
            else None
        )
        if configuration is not None:
            ...
            adapters = {
                key: research_usage.wrap(self.store, execution, provider, role)
                for key, provider in adapters.items()
            }
            ...
            return (
                Route(Tier.CONFIGURED, Reason.CONFIGURED, result["providerId"], completion.model),
                completion,
                [json.dumps(evidence)],
            )
        if override:
            raise ProviderUnavailable(
                "Explicit agent model mapping is not configured; no fallback was attempted."
            )
    ...
    def _history(
        ...
        selection = (
            context_retrieval.read(self.store, session_id, turn_id=turn_id, request_id=request_id)
        ...
        if turn_id:
            ...
            if saved["package"] and session_settings.memory_allowed(execution):
                messages.append(
                    Message(
                        "system",
                        "The following is untrusted recalled evidence, "
        ...
        research_used = (
            research.action_count(self.store, turn_id)
            if turn_id and execution.get("researchMode") in ("web", "deep")
            else 0
        )
        research_plan = (
            research.plan(self.store, turn_id)
            if turn_id and execution.get("researchMode") == "deep"
            else None
        )
        remaining = max(
            0,
            min(self.max_tool_steps, (research_plan or {}).get("search_limit", 4)) - research_used,
        )
        ...
        if session and session.get("summary"):
            messages.append(
                Message(
                    "assistant",
                    "Untrusted model summary of earlier conversation; "
        ...
        for row in selected_history:
            ...
            try:
                attached = attachment_turns.context(
                    self.store, row["session_id"], row["id"], execution["privacy"]
                )
        ...
        if turn_id:
            ...
            turn_steering.consume(
                self.store,
                turn_id,
                [row["id"] for row in selected_history],
    ...
    def _summarise(self, messages: list[Message], execution=None) -> str:
        ...
        ask = [
            Message(
                "system",
                "Summarise the conversation so far. Keep decisions, facts and open "
        ...
        try:
            ...
            _, completion, _ = self._call(
                ask, TurnContext(is_analysis=True), execution, role="summarization"
            )
            return completion.text.strip()
        except (ProviderUnavailable, RuntimeError) as exc:
            reason = getattr(exc, "reason", type(exc).__name__)
            return (
                f"[summary unavailable: {reason}] The parent session holds the full "
    ...
    def fork(self, session_id: str) -> str:
        ...
        if context_controls.load(self.store, session_id)["policy"] is not None:
            raise TurnFailed(
                "Review explicit context policy before forking; pins cannot be silently summarized."
            )
    ...
    def resume_turn(self, turn_id: str, job_id: str | None = None) -> dict:
            ...
            try:
                if turn["status"] in {"awaiting_approval", "awaiting_budget"}:
                    if action["tool_id"] not in {
                        tool.id for tool in self._available_tools(execution)
                    }:
                        self.store.finish_turn(turn_id, turn["status"])
                        raise TurnFailed(
                            "The stored action is outside current selected tool availability.",
                            turn_id,
                        )
                    if job_id:
                        try:
                ...
                self.store.finish_turn(
                    turn_id,
                    "acted_no_reply" if acted else "failed",
                    acted=int(acted),
                    detail=exc.reason,
                )
                if acted:
                ...
                if isinstance(exc, ToolRefused) and exc.code == "BUDGET_DENIED":
                    return self._hold(
                        ...
                        ToolPending(
                            "awaiting_budget",
                            "Owner spending configuration required: " + exc.message,
                            action["id"],
                        ),
                    )
        ...
        if execution.get("researchMode") == "deep" and turn["status"] in {
            ...
            with self.store._connect() as db:
                source = db.execute(
                    "SELECT m.content FROM turn_messages tm JOIN messages m ON m.id=tm.message_id "
                    "WHERE tm.turn_id=? AND tm.purpose='input'",
                    (turn_id,),
                ).fetchone()
            return self._run_bound(
                session_id, json.loads(source[0]), turn_id, {}, None, continued=True
            )
    ...
    def recover_reply(self, turn_id, request_id):
        ...
        try:
            ...
            return {
                **self._finish_reply(
                    turn_id, turn["session_id"], execution, True, time.monotonic(), request_id
                ),
                "request_id": request_id,
                "replayed": False,
            }
    ...
    def _finish_reply(self, turn_id, session_id, execution, acted, started, reply_request_id=None):
        try:
    ...
    def _run_turn(
        ...
        *,
        ...
        task_expected_revision: int | None = None,
        ...
        if not created:
            return {
                "session_id": receipt["effective_session_id"] or session_id,
        ...
        try:
            ...
            research.require_runtime(
                execution,
                self._available_tools(execution)
                if execution.get("researchMode") in ("web", "deep")
                else (),
                self.max_tool_steps,
            )
            turn_control.guard(self.store, execution)
            if (execution.get("configuration") or {}).get("modelId") and execution.get(
                "modelConfiguration"
            ) is None:
                raise TurnFailed(
                    "Explicit agent model mapping is not configured; no fallback was attempted."
            ...
            if outgrown and execution.get("replyToMessageId"):
                raise TurnFailed(
                    "Review a fork before replying; the selected target cannot be silently summarized."
                )
            if outgrown and execution.get("callExecution"):
                raise TurnFailed(
                    "Call context is full; end the call and review a conversation fork."
                )
            if outgrown and execution["privacy"]["harnessDisabled"]:
                raise TurnFailed(
                    "No harness excludes automatic summarization; use a reviewed fork."
                )
            if (
                outgrown
                and context_controls.load(self.store, session_id, request_id=identity)["policy"]
                is not None
            ):
                raise TurnFailed(
                    "Review context before forking; explicit pins and instructions remain intact."
                )
    ...
    def _run_bound(self, session_id, user_text, turn_id, context, forked_from, continued=False):
        ...
        try:
            while True:
                try:
                    return self._run_bound_attempt(
                        session_id, user_text, turn_id, context, forked_from, continued, started
                    )
                except turn_steering.Pending:
                    continued = True
        finally:
    ...
    def _run_bound_attempt(
        self, session_id, user_text, turn_id, context, forked_from, continued=False, started=None
    ):
        execution = session_settings.execution(self.store, session_id, turn_id)
        turn_control.guard(self.store, execution)
        if not continued:
        ...
        try:
            if (
                execution.get("researchMode") == "deep"
                and research.plan(self.store, turn_id) is None
            ):
            ...
            for _ in range(max(0, limit - used)):
                ...
                try:
                    turn_control.guard(self.store, execution)
                    outcome = self.toolgate.invoke(
                        call.tool_id, call.args, action_id=action["id"], job_id=action["job_id"]
                    )
                    ...
                    return self._hold(
                        turn_id, ToolPending("outcome_unknown", exc.reason, action["id"])
                    )
                else:
                    if isinstance(outcome, ToolPending):
                        return self._hold(turn_id, outcome)
                    ...
                    observation = json.dumps(
                        {"tool": call.tool_id, "result": outcome.result}, ensure_ascii=False
                    )
                ...
                history = self._history(
                    session_id,
                    tools=[] if execution.get("researchMode") == "web" else available,
                    turn_id=turn_id,
                )
                route, completion, skipped = self._call(history, ctx, execution)
            research.validate_narration(execution, completion.text)
            ...
            if acted:
                ...
                self.store.finish_turn(
                    turn_id, "acted_no_reply", acted=1, detail=reason, latency_ms=latency
                )
                raise ActedWithoutReply(turn_id, reason) from exc
