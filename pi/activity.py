...
def _run(db, row):
    ...
    value = {
        key: row[key]
        for key in ("id", "session_id", "status", "provider", "model", "started_at", "ended_at")
    }
    value.update(
        ...
        content_status="available" if _available(db, row["session_id"]) else "forgotten",
    )
    action = db.execute(
        "SELECT id,state,job_id FROM tool_actions WHERE turn_id=? ORDER BY rowid DESC LIMIT 1",
        (row["id"],),
    ).fetchone()
...
def list_runs(store, limit=50, cursor=None, session_id=None, task_id=None):
    ...
    with store._connect() as db:
        ...
        rows = db.execute(
            "SELECT * FROM turns" + clause + " ORDER BY started_at DESC,id DESC LIMIT ?",
            (*values, limit + 1),
        ).fetchall()
        page = rows[:limit]
        return {
            "results": [_run(db, row) for row in page],
...
def list_events(store, limit=50, cursor=None, session_id=None, task_id=None, run_id=None):
    ...
    with store._connect() as db:
        ...
        if task_id:
            ...
            where.append(
                "(task_id=? OR (task_id IS NULL AND run_id IN "
        ...
        clause = " WHERE " + " AND ".join(where) if where else ""
        rows = db.execute(
            "SELECT * FROM activity_events" + clause + " ORDER BY sequence DESC LIMIT ?",
            (*values, limit + 1),
        ).fetchall()
        page = rows[:limit]
        return {
            "results": [
                {
                    **dict(row),
                    ...
                    if _available(db, row["session_id"])
                    else "forgotten",
                }
                for row in page
            ],
