...
class DecisionProvider:
    ...
    def health(self):
        try:
            with httpx.Client(transport=self.transport, trust_env=False, timeout=2,
    ...
    def choose(self, state, instructions, choices, *, timeout=2.0):
        try:
            ...
            if (data["choice"] not in choices or type(data["confidence"]) not in {int, float}
    ...
    def rank_memories(self, query, package):
        ...
        if 2 <= len(records) <= 8:
            try:
                ...
                previews = {f"m{i}": (record.get("summary") or record.get("text") or "")[:160]
                            for i, record in enumerate(records)}
                data = self.choose(json.dumps({"query": query, "memory_previews": previews}, ensure_ascii=False),
                ...
                receipt.update(status="ranked", model=data["model"], confidence=data["confidence"],
                               previewCharacters=160,
                               order=[records[i]["id"] for i in indices])
            except (ProviderUnavailable, TypeError, KeyError, ValueError):
                receipt.update(status="fallback", reason="Decision service unavailable or input unsupported.")
...
def configured(environment):
    ...
    if not url and not key:
        return {}
    return {"decisions": DecisionProvider(url, key, minimum_confidence=float(
        environment.get("PI_DECISION_MIN_CONFIDENCE", "0.2")))}
