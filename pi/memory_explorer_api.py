...
Kind = Literal[
    "memory", "entity", "evidence", "analysis", "episode", "observation", "pattern", "transcript"
]
...
def router(memory, authorize):
    ...
    def inspect(operation, payload):
        ...
        try:
            ...
            if exc.response.status_code in {404, 422}:
                raise HTTPException(
                    exc.response.status_code, "Memory record or field is unavailable."
                ) from None
    ...
    def library(
        ...
        object_type: Kind | None = None,
        after: str | None = Query(default=None, max_length=240),
        limit: int = Query(default=25, ge=1, le=50),
    ):
        return inspect(
            "library",
            {"search": search, "object_type": object_type, "after": after, "limit": limit},
        )
